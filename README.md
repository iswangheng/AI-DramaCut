...
> 📖 **详细依赖配置指南**: 查看 [`docs/DEPENDENCIES.md`](docs/DEPENDENCIES.md) 获取完整的依赖安装说明、版本要求和常见问题解决方案。

**必需软件**：
- **Node.js** >= 18.0.0（推荐 20.x）
- **Python** >= 3.9（推荐 3.10+，用于 Whisper 语音识别）
- **Redis** >= 6.0（任务队列必需）
- **FFmpeg** >= 5.0（视频处理必需）
- **npm** >= 9.0.0
...
# 安装 Python 依赖（OpenAI Whisper - 用于语音转文字）
python3 -m pip install openai-whisper
```

**Python 依赖说明**：
- `openai-whisper`: ~150 MB（不含模型）
- 首次运行会自动下载 `base` 模型（~150 MB）
- 详细说明：[`docs/DEPENDENCIES.md`](docs/DEPENDENCIES.md)

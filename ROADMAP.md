...
| API 客户端 | ✅ 完成 | 100% | Gemini 3、ElevenLabs |
| 数据库层 | ✅ 完成 | 100% | SQLite + Drizzle ORM |
| 任务队列 | 🟡 部分完成 | 50% | BullMQ 配置完成，Worker 待开发 |
| WebSocket | 🟡 部分完成 | 50% | 服务器创建，待集成 |
| 前端 UI | 🔴 未开始 | 0% | 需要开发 |
| 业务逻辑 | 🔴 未开始 | 0% | 模式 A、模式 B |
...
|---------|------|--------|----------|------|
| P2-A1 | Gemini 高光检测 API | 🔴 最高 | 2小时 | Gemini 客户端（已完成）|
| P2-A2 | 高光片段提取 | 🔴 最高 | 3小时 | P0-3 视频拼接（已完成）|
| P2-A3 | 毫秒级微调 UI | 🟡 高 | 1天 | 无 |
| P2-A4 | 实时预览功能 | 🟡 高 | 1天 | P2-A3 |
| P2-A5 | 切片导出功能 | 🟡 高 | 2小时 | P0-2 进度监控（已完成）|
...
[P2-A1] 调用 Gemini findHighlights()
    返回: [
      { timestamp: 30000, type: 'reversal', confidence: 0.95 },
      { timestamp: 85000, type: 'conflict', confidence: 0.92 }
    ]
    ↓
...
| 任务 ID | 功能 | 优先级 | 预估工期 | 依赖 |
|---------|------|--------|----------|------|
| P2-B1 | 故事线提取 API | 🔴 最高 | 2小时 | Gemini 客户端（已完成）|
| P2-B2 | 解说文案生成 | 🔴 最高 | 2小时 | Gemini 客户端（已完成）|
| P2-B3 | ElevenLabs TTS 集成 | 🔴 最高 | 3小时 | ElevenLabs 客户端（已完成）|
| P2-B4 | 语义搜索系统 | 🟡 高 | 2天 | 向量数据库（未安装）|
| P2-B5 | 自动音画匹配 | 🟢 中 | 1天 | P2-B4 |
| P2-B6 | 多片段渲染 | 🟡 高 | 3小时 | P1-3 多片段组合（已完成）|
| P2-B7 | 四轨道混音 | 🟡 高 | 2小时 | P1-1 多轨道混音（已完成）|
